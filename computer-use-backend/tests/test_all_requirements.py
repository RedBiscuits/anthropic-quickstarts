#!/usr/bin/env python3
"""
Comprehensive test script to verify all CambioML requirements are met
"""
import asyncio
import json
import requests
...
class RequirementTester:
    ...
    def test_requirement_2_3_vnc_connection(self) -> bool:
        """Test Requirement 2.3: VNC connection to virtual machine"""
        self.print_header("Requirement 2.3: VNC Connection")
        
        try:
            # Test VNC server accessibility
            response = requests.get(VNC_URL, timeout=5)
            self.print_result("VNC Server Access", response.status_code == 200,
                            f"Status: {response.status_code}")
            
            # Test noVNC interface
            novnc_response = requests.get(f"{VNC_URL}/vnc.html", timeout=5)
            self.print_result("noVNC Interface", novnc_response.status_code == 200,
                            f"Status: {novnc_response.status_code}")
            ...
            try:
                ...
                sock.close()
                self.print_result("VNC Port 5900", result == 0,
                                f"Port status: {'Open' if result == 0 else 'Closed'}")
            except Exception as e:
                self.print_result("VNC Port 5900", False, f"Error: {e}")
            
            return all([
                self.test_results.get("VNC Server Access", False),
                self.test_results.get("noVNC Interface", False),
                self.test_results.get("VNC Port 5900", False)
            ])
    ...
    def test_requirement_3_docker_setup(self) -> bool:
        ...
        try:
            # Test if services are running on expected ports
            ports_to_test = [
                (8000, "Backend API"),
                (6080, "noVNC Web Interface"),
                (5900, "VNC Server"),
                (8080, "Agent HTTP Server")
            ]
            ...
            for port, service_name in ports_to_test:
                try:
                    ...
                    sock.close()
                    self.print_result(f"{service_name} (Port {port})", result == 0,
                                    f"Status: {'Open' if result == 0 else 'Closed'}")
                    if result != 0:
                        all_ports_ok = False
                except Exception as e:
                    self.print_result(f"{service_name} (Port {port})", False, f"Error: {e}")
                    all_ports_ok = False
